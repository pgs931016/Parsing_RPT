...
data_folder = 'H:\공유 드라이브\BSL-Data\Processed_data\Hyundai_dataset\RPT_data(Formation,OCV,DCIR,C-rate,GITT,RPT)\C_rate_data\C_rate\HNE_AHC_(5)_C_rate';
OCV_fullpath = 'H:\공유 드라이브\BSL-Data\Processed_data\Hyundai_dataset\Coin_cell\OCV\AHC_(5)_OCV_C20.mat';
np = -1; % 1 for CHC and FC, -1 for AHC
...
% C-rate test filees
files = dir([data_folder filesep 'HNE_AHC_01_C_rate_001.mat']);
...
for i = 1
    ...
    for j = 1:length(data)
        ...
        nth_chg = find(abs(crate_chg_vec - data(j).Iavg/I_1C) < 0.1);
        nth_dis = find(abs(crate_dis_vec - data(j).Iavg/I_1C) < 0.1);
    ...
    legend({'C/20','C/10','C/2','1C','2C','4C','6C'},'location','northeast')
...
xlim([0 1]);
ylim([-0.3 1])
title('C-rate(Charge) CH1');
set(gca,'FontSize',15)
box on;

   fig1 = fullfile(data_folder,'C-rate_cell1.fig');
   saveas(gcf, fig1);
