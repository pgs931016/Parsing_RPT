...
for n = 1:length(merged_files)
  % 데이터 불러오기
    ...
    for l = 1:length(data_merged)
        ...
        I_1C = 0.00429; %[A]
        data_merged(l).Iavg = mean(data(l).I); 
        if data_merged(l).rptflag == 1 && abs(Vmin - data_merged(l).V(end)) < 10e-2 && abs(cutoff - data(l).Iavg) < 10e-2 
            indices = [indices, l];
            indices = indices(1)+1
        end
        ...
        % data_merged(step_ocv_chg).soc = data(step_ocv_chg).cumQ/data(step_ocv_chg).Q; 
        % data_merged(step_ocv_dis).soc = data(step_ocv_dis).cumQ/data(step_ocv_dis).Q;
