...
for j = 1:length(data)
    ...
    if abs(Vmin - data(j).V(end)) < 0.001 && abs(cutoff1 - data(j).Iavg/I_1C) < 0.001 &&  j>index1    
    indices = [indices, j];
    index2 = indices(1);
    disp(['Value found at index ', num2str(j)]);
    end
...
for j = 1:length(data)
    data(j).Iavg = mean(data(j).I);
    if abs(Crate - data(j).Iavg/I_1C) < 0.001
      disp(['Value found at indices ', num2str(j)]);
    indices = [indices, j];
    index3 = indices(1,1);
    end
end
...
indices = [];
for j = 1:length(data)
    if abs(Vmin - data(j).V(end)) < 0.001 && abs(cutoff2 - data(j).Iavg/I_1C) < 0.001 && j>index2 && j<index3
         disp(['Value found at index ', num2str(j)]);
    indices = [indices, j];
    index4 = indices(1)
    end
