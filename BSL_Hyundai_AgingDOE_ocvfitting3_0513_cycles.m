...
% hyperparameters
    ...
    N_multistart = 24;
    t_pause_plot = 0;
    %Crate_Q = [56.2549, 54.9090, 54.3720, 53.7353, 52.9119]; %1c1c
    %Crate_Q = [56.5790, 55.9170 55.5610 55.0090 54.1250 51.4230 25.1460];%cycles till 600
    Crate_Q = [56.5790, 55.9170 55.5610 55.0090 54.1250 51.4230];%cycles till 500
    %Crate_Q = [57.2223 55.8310 55.4425 54.7317 53.6671]; 
    %Crate_Q = [56.2549 51.8287 50.5479 53.9725 40.2882 36.1870 39.1100];%cells 7cells
...
% X_numeric = cellfun(@str2double, [data_ocv.cycle]);

% X_cell = {'NE-BOL', '200-2', '350-2', '400-2'};
% 
% data_ocv.cycle = [data_ocv(1:6).cycle];
% data_ocv.dQ_LLI = [data_ocv(1:6).dQ_LLI];
% data_ocv.dQ_LAMp = [data_ocv(1:6).dQ_LAMp];
% data_ocv.dQ_data = [data_ocv(1:6).dQ_data];
data_ocv(1).dQ_LLI = 0;
data_ocv(1).dQ_LAMp = 0;

% figure()
% bar([data_ocv(1:7).cycle],[[data_ocv(1:7).dQ_LLI];[data_ocv(1:7).dQ_LAMp];[abs(data_merged(4).Q)] - [Crate_Q] - [data_ocv(1:7).dQ_data]],'stacked')
% hold on
% plot([data_ocv(1:7).cycle], [data_ocv.dQ_data], '-sc', 'LineWidth', 2);
% plot([data_ocv(1:7).cycle], [data_ocv.dQ_data] + [abs(data_merged(4).Q)] - [Crate_Q]- [data_ocv.dQ_data],'-sm', 'LineWidth', 2)
...
% 
figure()
bar([data_ocv(1:6).cycle],[[data_ocv(1:6).dQ_LLI];[data_ocv(1:6).dQ_LAMp];[abs(data_merged(4).Q)] - [Crate_Q] - [data_ocv(1:6).dQ_data]],'stacked')
hold on
plot([data_ocv(1:6).cycle] , [data_ocv.dQ_data], '-sc', 'LineWidth', 2);
plot([data_ocv(1:6).cycle] , [data_ocv.dQ_data] + [abs(data_merged(4).Q)] - [Crate_Q]- [data_ocv.dQ_data],'-sm', 'LineWidth', 2)
%legend({'Loss by LLI', 'Loss by LAMp', 'Loss by R'},'Location', 'northwest'); 
legend({'Loss by LLI', 'Loss by LAMp', 'Loss by R', 'Loss data (C/10)', 'Loss data (C/3)'},'Location', 'northwest'); 
...
fig2 = sprintf('C:\\Users\\GSPARK\\Downloads\\caploss500cyc.fig');
saveas(gcf, fig2);
% % 
save_path = 'C:\\Users\\GSPARK\\Downloads\\NE_data_ocv2.mat';
data_merged = data_ocv(1:7); 
