...
% MAT 파일 가져오기
slash = filesep;
files = dir([data_folder slash '*CH4*.mat']);
...
for i = 1:length(BigI)
   data((BigI(i))).R01s = data(BigI(i)).R(1);
   data(BigI(i)).R10s = data(BigI(i)).R(52);
   data(BigI(i)).R30s = data(BigI(i)).R(end);
end

for i = 1:length(BigI)
   data(BigI(i)).R0 = data(BigI(i)).R(1);
   data(BigI(i)).R1 = data(BigI(i)).R(52) - data(BigI(i)).R0 ;
   data(BigI(i)).R30s = data(BigI(i)).R(end);
end
...
if id_cfa == 1 || id_cfa == 2
    for i = 1:length(BigIC)
        SOC01sc = [SOC01sc, data(BigIC(i)).SOC(1)];
        R01sc = [R01sc, data(BigIC(i)).R01s];

        % SOC1sc = [SOC1sc, data(BigIC(i)).SOC(11)];
        % R1sc = [R1sc, data(BigIC(i)).R1s];

        SOC10sc = [SOC10sc, data(BigIC(i)).SOC(52)];
    ...
    for i = 1 : length(BigID)
        ...
        R001sd = [R001sd, data(BigID(i)).R01s];
        % SOC1sd = [SOC1sd, data(BigID(i)).SOC(11)];
        % R1sd = [R1sd, data(BigID(i)).R1s];
        SOC10sd = [SOC10sd, data(BigID(i)).SOC(52)];
...
elseif id_cfa == 3
    for i = 1:length(BigI)
        SOC01sc = [SOC01sc, data(BigI(i)).SOC(2)];
        R001s = [R001s, data(BigI(i)).R01s];
        % SOC1s = [SOC1s, data(BigI(i)).SOC(11)];
        % R1s = [R1s, data(BigI(i)).R1s];
        SOC10s = [SOC10s, data(BigI(i)).SOC(52)];
...
smoothed_SOC_001sc = linspace(min(SOC01sc), max(SOC01sc), 100);
smoothed_R_001sc = spline(SOC01sc, R01sc, smoothed_SOC_001sc);
...
figure(1);
hold on;
plot(SOC01sc, R01sc, 'go');
plot(smoothed_SOC_001sc, smoothed_R_001sc,'-g');
% plot(SOC1sc, R1sc, 'o');
% plot(smoothed_SOC_1sc, smoothed_R_1sc);
plot(SOC10sc, R10sc, 'ro');
plot(smoothed_SOC_10sc, smoothed_R_10sc,'-r');
plot(SOC30sc, R30sc, 'bo');
plot(smoothed_SOC_30sc, smoothed_R_30sc,'-b');
hold off;
...
legend('30s','', '10s','', '1ms','');
xlim([0 1]);
title('Resistance (Charge) CH4');
set(gca,'FontSize',15)
box on;

% % Generate smoothed data for 'sd' case
% smoothed_SOC_01sd = linspace(min(SOC01sd), max(SOC01sd), 100);
% smoothed_R_01sd = spline(SOC01sd, R01sd, smoothed_SOC_01sd);
% % 
...
% plot(smoothed_SOC_01sd, smoothed_R_01sd);
% plot(SOC1sd, R1sd, 'o');
