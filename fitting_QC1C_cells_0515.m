...
% hyperparameters
    ...
    N_multistart = 24;
    t_pause_plot = 0;
    % Crate_Q = [56.255 52.135 50.856 54.279 36.494 39.418 40.595];
    %Crate_Q = [56.255 50.5479 39.1100 40.2882];
    Crate_Q = [56.255 51.8287 50.5479 53.9725 36.1870 39.1100 40.2882];
...
% load Merged data
    ...
    %load("G:\공유 드라이브\BSL-Data\Processed_data\Hyundai_dataset\현대차파우치셀 (rOCV,Crate)\QC1C\QC1C셀들(C20)\NE_OCV_merged2.mat")
    %load("G:\공유 드라이브\BSL-Data\Processed_data\Hyundai_dataset\현대차파우치셀 (rOCV,Crate)\QC1C\QC1C사이클(C10)\NE_OCV_Merged.mat")
    ...
    load("G:\공유 드라이브\BSL-Data\Processed_data\Hyundai_dataset\현대차파우치셀 (rOCV,Crate)\QC1C\QC1C셀들(C20)\NE_OCV_merged.mat")
...
for j = 1:size(data_ocv,1)
    ...
    % % fig1 = sprintf('G:\\공유 드라이브\\BSL-Data\\Processed_data\\Hyundai_dataset\\현대차파우치셀 (rOCV,Crate)\\1C1C\\%d.jpg', j);
    % % fig1 = sprintf('G:\\공유 드라이브\\BSL-Data\\Processed_data\\Hyundai_dataset\\현대차파우치셀 (rOCV,Crate)\\QC1C\\QC1C사이클(C10)\\%d.jpg', j);
    % fig1 = sprintf('G:\\공유 드라이브\\BSL-Data\\Processed_data\\Hyundai_dataset\\현대차파우치셀 (rOCV,Crate)\\QC1C\\QC1C셀들(C20)\\%d.jpg', j);
    fig1 = sprintf('C:\\Users\\GSPARK\\Downloads\\%d.fig', j);
    saveas(gcf, fig1);
    pause(t_pause_plot)
...
hold on
% plot([data_ocv.dQ_data], '-sc', 'LineWidth', 2);
% plot([data_ocv.dQ_data] + [abs(data_merged(4).Q)] - [Crate_Q]- [data_ocv.dQ_data],'-sm', 'LineWidth', 2)
% legend({'Loss by LLI', 'Loss by LAMp', 'Loss by R', 'Loss data (C/20)', 'Loss data (C/3)'},'Location', 'northwest'); 
legend({'Loss by LLI', 'Loss by LAMp', 'Loss by R'},'Location', 'northwest'); 
ylim([0 25]);
xlabel('Cell lndex');
ylabel('$\Delta Q$ [Ah]', 'Interpreter', 'latex','FontName', 'Arial','FontWeight', 'bold');
% 
title('고속충전(QC1C) 대형셀 열화 인자');
% %fig2 = sprintf('G:\\공유 드라이브\\BSL-Data\\Processed_data\\Hyundai_dataset\\현대차파우치셀 (rOCV,Crate)\\1C1C\\bar_plot.jpg');
% %fig2 = sprintf('G:\\공유 드라이브\\BSL-Data\\Processed_data\\Hyundai_dataset\\현대차파우치셀 (rOCV,Crate)\\QC1C\\QC1C셀들(C20)\\bar_plot2.jpg');
fig2 = sprintf('C:\\Users\\GSPARK\\Downloads\\caploss_bar2.fig');
saveas(gcf, fig2);
% 
% %save_path = 'G:\공유 드라이브\BSL-Data\Processed_data\Hyundai_dataset\현대차파우치셀 (rOCV,Crate)\1C1C\NE_data_ocv';
% %save_path = 'G:\공유 드라이브\BSL-Data\Processed_data\Hyundai_dataset\현대차파우치셀 (rOCV,Crate)\QC1C\QC1C사이클(C10)\NE_data_ocv';
% %save_path = "G:\공유 드라이브\BSL-Data\Processed_data\Hyundai_dataset\현대차파우치셀 (rOCV,Crate)\QC1C\QC1C셀들(C20)\NE_data_ocv2";
% save_path = "C:\Users\GSPARK\Downloads\NE_data_ocv";
