% clc;clear;close all
% 
% folder_path = 'H:\공유 드라이브\BSL_Data2\HNE_AgingDOE_Processed\HNE_FCC\1CPD 0.5C (25-42)\n10degC';
...
rootfolder_path = 'H:\공유 드라이브\BSL_Data2\HNE_AgingDOE_Processed\HNE_FCC';
    ...
    for m = 1:length(folder_list)
        if ~isempty(folder_list{m}) ...
            ...
            disp(['Processing folder: ' folder_list{m}]); % 디버깅 출력 추가
...
disp(['Number of merged files found: ' num2str(length(merged_files))]);
  ...
  % 데이터 불러오기
    ...
    for l = 1:length(data_merged)
       ...
       data_merged(l).Iavg = mean(data_merged(l).I); 
        ...
        data_merged(l).Q = 0;
        data_merged(l).cumQ = 0;
        data_merged(l).soc = data_merged(l-1).soc(end);
        end
       ...
       elseif data_merged(l).rptflag == 1 && abs(Vmin - data_merged(l).V(end)) < 10e-3 && abs(cutoff_min - data_merged(l).Iavg/I_1C) < 10e-3 && data_merged(l-2).type == 'C'
        ...
        if data_merged(l).OCVflag ==1 
        data_merged(l).soc = data_merged(l).cumQ/data_merged(l).Q;
        elseif data_merged(l).OCVflag ==2
        data_merged(l).soc = 1-abs(data_merged(l).cumQ)/abs(data_merged(l).Q);
        else data_merged(l).soc = NaN;
        end
...
plot([data_D_Aging.cycle],abs([data_D_Aging.Q]),'ok');hold on;
legend(newNamePart)

figure(3)
data_D_RPT = data_merged(([data_merged.type]=='D')&(abs([data_merged.Q])>0.003)&([data_merged.OCVflag]==2));
plot([data_D_RPT.cycle],abs([data_D_RPT.Q]),'or');hold on;
legend(newNamePart)
end
